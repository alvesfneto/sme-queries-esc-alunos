...
RUA ALBA VALDEZ, 05 - JD REIMBERG(-23.751343376867045, -46.69657441887718)
...
*/
DROP TABLE ##alunos_infantil_distantes_ate_1km_RUA_ALBA_VALDEZ_05_JD_REIMBERG
SELECT 
ETAPA,
SERIE_ANO,
NOME_ALUNO,
...
FLOOR(FLOOR(SQRT(POWER((LAT_ALUNO*1000000 - (-23.751343376867045 * 1000000) ),2) +  POWER((LON_ALUNO*1000000  - (-46.69657441887718 * 1000000)),2)) / 10)/0.6) AS 'DIST_RUA_ALBA_VALDEZ_05_JD_REIMBERG',
CASE WHEN DEF1 IS NOT NULL THEN 'S' ELSE NULL END AS 'DEFICIENCIA',
CASE WHEN TEG ='SIM' THEN 'S' ELSE NULL END AS 'TEG',
A.CRDATE

INTO ##alunos_infantil_distantes_ate_1km_RUA_ALBA_VALDEZ_05_JD_REIMBERG
...
AND FLOOR(FLOOR(SQRT(POWER((LAT_ALUNO*1000000 - (-23.751343376867045 * 1000000) ),2) +  POWER((LON_ALUNO*1000000  - (-46.69657441887718 * 1000000)),2)) / 10)/0.6)<=1000

--SELECT * FROM ##alunos_infantil_distantes_ate_1km_RUA_ALBA_VALDEZ_05_JD_REIMBERG